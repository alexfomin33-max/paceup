// ────────────────────────────────────────────────────────────────────────
// Управление задачами через PHP API
// ────────────────────────────────────────────────────────────────────────
...
class TaskManager {
  ...
  // ────────────────────────────────────────────────────────────────────────
  // Загрузка и сохранение через PHP API
  // ────────────────────────────────────────────────────────────────────────

  async loadTasks() {
    try {
      const response = await fetch('index.php?action=get');
      ...
      if (result.success) {
        ...
        console.error('Ошибка загрузки задач:', result.message);
      }
    } catch (error) {
    ...
    if (result.success) {
      ...
      if (result.task) {
        requestAnimationFrame(() => {
          const taskItem = document.querySelector(`[data-task-id="${result.task.id}"]`);
          if (taskItem) {
  ...
  async completeTask(id) {
    ...
    if (taskItem) {
      ...
      setTimeout(async () => {
        const result = await this.apiCall('complete', { id });
        if (result.success) {
          await this.loadTasks();
        } else {
          alert('Ошибка: ' + result.message);
  ...
  async restoreTask(id) {
    ...
    const result = await this.apiCall('restore', { id });
    if (result.success) {
      await this.loadTasks();
    } else {
      alert('Ошибка: ' + result.message);
  ...
  async editTask(id, newText) {
    ...
    const result = await this.apiCall('edit', {
      id: id,
      text: newText.trim()
    });
  ...
  renderTasks() {
    ...
    activeTasks.forEach((task) => {
      activeList.appendChild(this.createTaskElement(task, false));
    });
  ...
  startEditing(id, textElement) {
    ...
    const finishEditing = async () => {
      ...
      if (newText !== currentText && newText !== '') {
        ...
        // Если текст не изменился или пустой, просто восстанавливаем элемент
  ...
  selectIcon(iconType) {
    if (this.selectedIcon === iconType) {
      // Если иконка уже выбрана, снимаем выбор
      this.selectedIcon = null;
    } else {
