...
class _RouteCardState extends State<RouteCard> {
  ...
  // ────────────────────────────────────────────────────────────────
  // 🔹 КЕШИРОВАНИЕ УПРОЩЕННОЙ ПОЛИЛИНИИ: для оптимизации производительности
  // ────────────────────────────────────────────────────────────────
  List<LatLng>? _cachedSimplifiedPoints;
  int? _cachedOriginalPointsLength;
  ...
  Widget build(BuildContext context) {
    ...
    // ────────────────────────────────────────────────────────────────
    // 🔹 УПРОЩЕНИЕ ПОЛИЛИНИИ: применяем алгоритм Дугласа-Пекера
    // ────────────────────────────────────────────────────────────────
    // Кешируем результат упрощения для оптимизации производительности
    // Упрощаем только если количество точек изменилось
    final simplifiedPoints = _getSimplifiedPoints(points);

    // Используем упрощенные точки для вычисления центра и границ
    // (это быстрее и не влияет на визуальное качество)
    final center = _centerFromPoints(simplifiedPoints);
    final bounds = _boundsFromPoints(simplifiedPoints);
            ...
            children: [
              ...
              IgnorePointer(
                ...
                child: AnimatedOpacity(
                  ...
                  child: MapWidget(
                    key: ValueKey('route_card_${simplifiedPoints.length}'),
                    onMapCreated: (MapboxMap mapboxMap) async {
                      ...
                      try {
                        ...
                        // ────────────────────────────────────────────────────────────────
                        // 🔹 ИСПОЛЬЗУЕМ УПРОЩЕННЫЕ ТОЧКИ: для оптимизации рендеринга
                        // ────────────────────────────────────────────────────────────────
                        // Создаем полилинию из упрощенных точек
                        final coordinates = simplifiedPoints
  ...
  /// Получает упрощенную полилинию с кешированием результата.
  ///
  /// ⚡ PERFORMANCE OPTIMIZATION:
  /// - Кеширует результат упрощения для избежания повторных вычислений
  /// - Пересчитывает только если количество точек изменилось
  /// - Использует адаптивный порог упрощения в зависимости от размера маршрута
  ///
  /// Параметры упрощения:
  /// - tolerance: 5 метров (оптимально для карточек 200px высотой)
  /// - maxPoints: 300 (достаточно для визуального качества)
  List<LatLng> _getSimplifiedPoints(List<LatLng> originalPoints) {
    // ────────────────────────────────────────────────────────────────
    // 🔹 КЕШИРОВАНИЕ: переиспользуем результат, если точки не изменились
    // ────────────────────────────────────────────────────────────────
    if (_cachedSimplifiedPoints != null &&
        _cachedOriginalPointsLength == originalPoints.length) {
      return _cachedSimplifiedPoints!;
    }

    // ────────────────────────────────────────────────────────────────
    // 🔹 УПРОЩЕНИЕ: применяем алгоритм Дугласа-Пекера
    // ────────────────────────────────────────────────────────────────
    // Для карточек в ленте используем:
    ...
    final simplified = PolylineSimplifier.simplify(
      ...
      tolerance: 5.0, // метры
      maxPoints: 300, // максимальное количество точек
    );

    // ────────────────────────────────────────────────────────────────
    // 🔹 СОХРАНЯЕМ В КЕШ: для последующего использования
    // ────────────────────────────────────────────────────────────────
    _cachedSimplifiedPoints = simplified;
    _cachedOriginalPointsLength = originalPoints.length;
