// lib/core/utils/static_map_url_builder.dart
import 'package:latlong2/latlong.dart';
import 'package:flutter/material.dart';
import '../config/app_config.dart';
import '../theme/app_theme.dart';

/// Утилита для генерации URL статичных карт Mapbox Static Images API.
///
/// Используется для создания статичных изображений маршрутов вместо
/// интерактивных карт Mapbox GL, что устраняет jank при скролле в ленте.
///
/// ⚡ PERFORMANCE OPTIMIZATION:
/// - Статичные PNG картинки загружаются быстрее, чем GL-поверхности
/// - Кеширование через CachedNetworkImage снижает повторные запросы
class StaticMapUrlBuilder {
  /// Генерирует URL для статичной карты Mapbox с маршрутом.
  ///
  /// Параметры:
  /// - [points] - список точек маршрута
  /// - [widthPx] - ширина изображения в пикселях
  /// - [heightPx] - высота изображения в пикселях
  /// - [strokeColor] - цвет линии маршрута (по умолчанию AppColors.polyline)
  /// - [strokeWidth] - ширина линии (по умолчанию 3)
  /// - [padding] - отступы от краёв в процентах от размера изображения (по умолчанию 15% для предотвращения обрезания трека)
  /// - [maxWidth] - максимальная ширина изображения (по умолчанию 800px для оптимизации)
  /// - [maxHeight] - максимальная высота изображения (по умолчанию 600px для оптимизации)
  ///
  /// Возвращает URL для Mapbox Static Images API.
  ///
  /// ⚡ PERFORMANCE OPTIMIZATION:
  /// - Ограничивает максимальный размер изображения для уменьшения размера файла
  /// - Сохраняет пропорции при ограничении размера
  /// - Расширяет bounding box для предотвращения обрезания трека
  static String fromPoints({
    required List<LatLng> points,
    required double widthPx,
    required double heightPx,
    Color? strokeColor,
    double strokeWidth = 3.0,
    double padding = 15.0,
    double? maxWidth,
    double? maxHeight,
  }) {
    // ────────────────────────────────────────────────────────────────
    // 🔹 ЗАЩИТА: обрабатываем edge cases
    // ────────────────────────────────────────────────────────────────
    if (points.isEmpty) {
      throw ArgumentError('points не может быть пустым');
    }

    // ────────────────────────────────────────────────────────────────
    // 🔹 КОДИРОВАНИЕ ПОЛИЛИНИИ: в формат Google polyline
    // ────────────────────────────────────────────────────────────────
    // Используем оригинальные точки маршрута без изменений
    // Padding будет добавлен через параметр URL Mapbox API
    final encodedPolyline = _encodePolyline(points);

    // ────────────────────────────────────────────────────────────────
    // 🔹 ЦВЕТ ЛИНИИ: используем polyline по умолчанию
    // ────────────────────────────────────────────────────────────────
    final color = strokeColor ?? AppColors.polyline;
    final colorHex = _colorToHex(color).toUpperCase();

    // ────────────────────────────────────────────────────────────────
    // 🔹 ОПТИМИЗАЦИЯ РАЗМЕРА: ограничиваем максимальные размеры для уменьшения веса файла
    // ────────────────────────────────────────────────────────────────
    // Для карточек в ленте используем размеры до 1200x900px для лучшего качества
    // на Retina-дисплеях (DPR 3.0) без избыточного размера файла
    final effectiveMaxWidth = maxWidth ?? 1200.0;
    final effectiveMaxHeight = maxHeight ?? 900.0;

    double finalWidth = widthPx;
    double finalHeight = heightPx;

    // Если размеры превышают максимум - масштабируем с сохранением пропорций
    if (finalWidth > effectiveMaxWidth || finalHeight > effectiveMaxHeight) {
      final widthRatio = effectiveMaxWidth / finalWidth;
      final heightRatio = effectiveMaxHeight / finalHeight;
      final scale = widthRatio < heightRatio ? widthRatio : heightRatio;

      finalWidth = (finalWidth * scale).roundToDouble();
      finalHeight = (finalHeight * scale).roundToDouble();
    }

    // ────────────────────────────────────────────────────────────────
    // 🔹 ФОРМИРОВАНИЕ URL: Mapbox Static Images API
    // ────────────────────────────────────────────────────────────────
    // Формат: https://api.mapbox.com/styles/v1/mapbox/{style}/static/{overlay}/{lon},{lat},{zoom}/{width}x{height}?access_token={token}
    // Или с path: https://api.mapbox.com/styles/v1/mapbox/{style}/static/{path-overlay}/auto/{width}x{height}?access_token={token}
    //
    // Path overlay формат: path-{strokeWidth}+{strokeColor}({encodedPolyline})
    // Используем auto для автоматического определения центра и зума по границам маршрута
    // Padding добавляется через параметр URL для предотвращения обрезания трека
    final pathOverlay = 'path-$strokeWidth+$colorHex($encodedPolyline)';
    final encodedPath = Uri.encodeComponent(pathOverlay);

    final width = finalWidth.round();
    final height = finalHeight.round();

    // Вычисляем padding в пикселях на основе процентов от размера изображения
    // Используем минимальный padding для маленьких изображений
    final paddingPx = ((width + height) / 2 * padding / 100.0).round().clamp(20, 80);
    
    // Формируем URL с параметром padding (в пикселях)
    // Padding применяется ко всем сторонам одинаково
    final url =
        'https://api.mapbox.com/styles/v1/mapbox/streets-v12/static/$encodedPath/auto/${width}x$height?padding=$paddingPx&access_token=${AppConfig.mapboxAccessToken}';

    return url;
  }

  /// Кодирует список точек в формат Google polyline (используется Mapbox).
  ///
  /// Алгоритм кодирования:
  /// 1. Преобразуем координаты в целые числа (умножаем на 1e5)
  /// 2. Вычисляем разности между соседними точками (delta encoding)
  /// 3. Кодируем каждую разность в base64-подобный формат (ASCII 63 символа)
  static String _encodePolyline(List<LatLng> points) {
    if (points.isEmpty) return '';

    final buffer = StringBuffer();
    int lastLat = 0;
    int lastLng = 0;

    for (final point in points) {
      // Преобразуем в целые числа (умножаем на 1e5 и округляем)
      final lat = (point.latitude * 1e5).round();
      final lng = (point.longitude * 1e5).round();

      // Вычисляем разности (delta encoding)
      final deltaLat = lat - lastLat;
      final deltaLng = lng - lastLng;

      // Кодируем разности
      _encodeValue(buffer, deltaLat);
      _encodeValue(buffer, deltaLng);

      lastLat = lat;
      lastLng = lng;
    }

    return buffer.toString();
  }

  /// Кодирует одно значение (разность координат) в формат Google polyline.
  ///
  /// Алгоритм:
  /// 1. Преобразуем отрицательные значения через битовый NOT и сдвиг
  /// 2. Разбиваем на 5-битовые блоки
  /// 3. Кодируем каждый блок в ASCII символ (63 символа, начиная с '?')
  static void _encodeValue(StringBuffer buffer, int value) {
    // Преобразуем отрицательные значения
    // Левый сдвиг на 1 бит, затем инвертируем если значение отрицательное
    int shifted = value << 1;
    if (value < 0) {
      shifted = ~shifted;
    }

    // Разбиваем на 5-битовые блоки и кодируем
    while (shifted >= 0x20) {
      // Берем младшие 5 бит и добавляем 63 (ASCII код '?')
      final char = (0x20 | (shifted & 0x1f)) + 63;
      buffer.writeCharCode(char);
      shifted >>= 5;
    }

    // Кодируем последний блок
    final char = shifted + 63;
    buffer.writeCharCode(char);
  }

  /// Преобразует Color в hex-строку без альфы (например, 379AE6).
  static String _colorToHex(Color color) {
    final r = ((color.r * 255.0).round() & 0xff)
        .toRadixString(16)
        .padLeft(2, '0');
    final g = ((color.g * 255.0).round() & 0xff)
        .toRadixString(16)
        .padLeft(2, '0');
    final b = ((color.b * 255.0).round() & 0xff)
        .toRadixString(16)
        .padLeft(2, '0');
    return '$r$g$b';
  }
}
