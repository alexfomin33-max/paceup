...
class CorosScreen extends ConsumerStatefulWidget {
  const CorosScreen({super.key});

  @override
  ConsumerState<CorosScreen> createState() => _CorosScreenState();
}

class _CorosScreenState extends ConsumerState<CorosScreen> {
  ...
  Future<void> _checkConnection() async {
    ...
    // TODO: Реализовать проверку подключения Coros через API
    ...
    setState(() {
      ...
      _status = _connected
          ? 'Coros подключен. Готов к синхронизации.'
          : 'Coros не подключен.';
    });
  }
  ...
  Future<void> _connectCoros() async {
    if (_requestingConnection) return;

    setState(() {
      _requestingConnection = true;
      _status = 'Подключение к Coros…';
    });

    try {
      // TODO: Реализовать OAuth авторизацию Coros
      ...
      setState(() {
        _connected = true;
        _status = 'Coros успешно подключен.';
        _requestingConnection = false;
      });

      _showSnackBar('Coros успешно подключен');
    } catch (e) {
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _syncCoros() async {
    if (_busy) return;

    setState(() {
      _busy = true;
      _status = 'Синхронизация с Coros…';
    });

    try {
      // TODO: Реализовать синхронизацию тренировок из Coros
      await Future.delayed(const Duration(seconds: 2));
      ...
      _showSnackBar('Тренировки успешно синхронизированы из Coros');
    } catch (e) {
      if (!mounted) return;
      setState(() {
                ...
                children: [
                  Row(
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          'Синхронизация с Coros. Подключите аккаунт Coros, чтобы импортировать тренировки, дистанцию, пульс и активные калории.',
            ...
            Center(
              child: PrimaryButton(
                ...
                onPressed: (_requestingConnection || _busy)
                    ? () {}
                    : (_connected ? _syncCoros : _connectCoros),
                width: 260,
                height: 44,
