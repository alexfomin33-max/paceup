...
class SuuntoScreen extends ConsumerStatefulWidget {
  const SuuntoScreen({super.key});

  @override
  ConsumerState<SuuntoScreen> createState() => _SuuntoScreenState();
}

class _SuuntoScreenState extends ConsumerState<SuuntoScreen> {
  ...
  Future<void> _checkConnection() async {
    ...
    // TODO: Реализовать проверку подключения SUUNTO через API
    ...
    setState(() {
      ...
      _status = _connected
          ? 'SUUNTO подключен. Готов к синхронизации.'
          : 'SUUNTO не подключен.';
    });
  }
  ...
  Future<void> _connectSuunto() async {
    if (_requestingConnection) return;

    setState(() {
      _requestingConnection = true;
      _status = 'Подключение к SUUNTO…';
    });

    try {
      // TODO: Реализовать OAuth авторизацию SUUNTO
      ...
      setState(() {
        _connected = true;
        _status = 'SUUNTO успешно подключен.';
        _requestingConnection = false;
      });

      _showSnackBar('SUUNTO успешно подключен');
    } catch (e) {
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _syncSuunto() async {
    if (_busy) return;

    setState(() {
      _busy = true;
      _status = 'Синхронизация с SUUNTO…';
    });

    try {
      // TODO: Реализовать синхронизацию тренировок из SUUNTO
      await Future.delayed(const Duration(seconds: 2));
      ...
      _showSnackBar('Тренировки успешно синхронизированы из SUUNTO');
    } catch (e) {
      if (!mounted) return;
      setState(() {
            ...
            Center(
              child: PrimaryButton(
                ...
                onPressed: (_requestingConnection || _busy)
                    ? () {}
                    : (_connected ? _syncSuunto : _connectSuunto),
                width: 260,
                height: 44,
