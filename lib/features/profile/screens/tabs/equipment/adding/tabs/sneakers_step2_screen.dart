...
import 'sneakers_step3_screen.dart';

/// Базовый URL изображений снаряжения (uploads.paceup.ru)
...
/// Модели загружаются из API (equip_base по brand + type=boots, status=1),
/// по алфавиту. Поиск фильтрует по вводу. Изображения из uploads (boots/id.ext).
/// ─────────────────────────────────────────────────────────────────────────────
  ...
  Future<void> _loadModels() async {
    ...
    try {
      ...
      final data = await api.post(
        ...
        body: {'brand': widget.brand, 'type': 'boots'},
      );
      if (!mounted) return;
...
/// Элемент списка модели: миниатюра из uploads (или плейсхолдер), название, галочка
