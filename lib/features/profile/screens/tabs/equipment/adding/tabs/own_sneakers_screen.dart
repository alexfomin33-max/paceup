// lib/features/profile/screens/tabs/equipment/adding/tabs/own_sneakers_screen.dart
import 'dart:io';
...
/// Экран «Добавить свои кроссовки» — для ввода произвольного бренда и модели
class OwnSneakersScreen extends ConsumerStatefulWidget {
  const OwnSneakersScreen({super.key});

  @override
  ConsumerState<OwnSneakersScreen> createState() =>
      _OwnSneakersScreenState();
}

class _OwnSneakersScreenState extends ConsumerState<OwnSneakersScreen> {
  // ─────────────────────────────────────────────────────────────────────
  //                             КОНТРОЛЛЕРЫ
  // ─────────────────────────────────────────────────────────────────────
  ...
  final _pickerFocusNode = FocusNode(debugLabel: 'ownSneakersPickerFocus');
  ...
  Widget build(BuildContext context) {
    ...
    final brand = _brandCtrl.text.trim();
    final model = _modelCtrl.text.trim();
    final isButtonEnabled = brand.isNotEmpty && 
                           model.isNotEmpty && 
                           _inUseFrom != null && 
...
class _RightTextFieldState extends State<_RightTextField> {
  ...
  Widget build(BuildContext context) {
    return TextField(
      ...
      onChanged: (_) {
        // ── Обновляем состояние родительского виджета для проверки валидности кнопки
