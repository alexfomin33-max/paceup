...
class _CommentsBottomSheetState extends ConsumerState<CommentsBottomSheet> {
  ...
  Future<void> _deleteComment(CommentItem comment) async {
    try {
      ...
      if (!ok) {
        throw Exception(
          (data['error'] ?? 'Не удалось удалить комментарий').toString(),
        );
      }
      ...
      if (mounted) {
        showSnack(context, 'Комментарий удален');
      }
    } catch (e) {
      if (mounted) {
        showSnack(context, 'Ошибка удаления: ${ErrorHandler.format(e)}');
      }
    }
  }
    ...
    if (isOwnComment) {
      ...
      items.add(
        MoreMenuItem(
          ...
          onTap: () async {
            // Меню автоматически закроется через MoreMenuOverlay
