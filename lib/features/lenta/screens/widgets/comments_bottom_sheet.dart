...
class _CommentsBottomSheetState extends ConsumerState<CommentsBottomSheet> {
  ...
  Future<void> _deleteComment(CommentItem comment) async {
    ...
    try {
      // ────────────────────────────────────────────────────────────────
      // 🔹 ApiService автоматически добавляет токен в заголовки
      // Если токен невалиден, сервер вернет ошибку, которую мы обработаем
      // ────────────────────────────────────────────────────────────────
      final api = ref.read(apiServiceProvider);
      final data = await api.post(
        '/comments_delete.php',
        body: {
          ...
          'userId': '${widget.currentUserId}', // 🔹 Передаем userId для проверки прав
        },
      );

      // ────────────────────────────────────────────────────────────────
      // ✅ Проверяем успешность операции
      // PHP возвращает булево значение true, поэтому используем строгую проверку
      // ────────────────────────────────────────────────────────────────
