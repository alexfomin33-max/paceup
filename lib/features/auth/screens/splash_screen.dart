...
class _SplashScreenState extends ConsumerState<SplashScreen>
  ...
  Future<void> _checkAuth() async {
    ...
    if (userId != null && accessToken != null && refreshToken != null) {
      ...
      // Это позволяет быстрее запускать приложение и не требовать авторизацию каждый раз
      // Валидация произойдет автоматически при первом запросе к API
      // Если токены невалидны - API вернет ошибку, и тогда обновим токены через refresh_token
      
      if (kDebugMode) {
        debugPrint(
          '✅ [Auth] Токены найдены в хранилище - переходим на ленту без валидации',
        );
        debugPrint(
      ...
      if (kDebugMode) {
        debugPrint(
          '🔹 Токены найдены, userId: $userId - переходим на основной экран',
        );
      }
