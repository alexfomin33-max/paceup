...
class _SplashScreenState extends ConsumerState<SplashScreen>
  ...
  Future<void> _checkAuth() async {
    ...
    // 🔹 Если токены есть - переходим на основной экран
    // Валидацию токена делаем в фоне (не блокируем переход)
    final int? userId = await auth.getUserId();
    if (!mounted) return;

    if (userId != null) {
      if (kDebugMode) {
        debugPrint('🔹 Токены найдены, userId: $userId - переходим на основной экран');
      }

      // Если интернет есть - валидируем токен в фоне (не блокируем переход)
      if (_hasInternet == true) {
        // Валидация в фоне, не ждем результата
        auth.validateToken().then((valid) {
          if (kDebugMode) {
            debugPrint('🔹 Валидация токена: ${valid ? "валиден" : "невалиден"}');
          }
          // Если токен невалиден, пользователь получит ошибку при первом запросе к API
        }).catchError((e) {
          if (kDebugMode) {
            debugPrint('⚠️ Ошибка валидации токена: $e');
          }
        });
      }
