    ...
    if (userId != null && accessToken != null && refreshToken != null) {
      ...
      if (_hasInternet == true) {
        try {
          ...
          if (kDebugMode) {
            debugPrint(
              '🔹 Валидация токена: ${isValid ? "валиден" : "невалиден"}',
            );
          }

          // 🔹 Если токен невалиден и не удалось обновить - переходим на экран входа
          if (!isValid) {
            if (kDebugMode) {
              debugPrint(
                '⚠️ Токен невалиден и не удалось обновить - переходим на экран входа',
              );
            }
          ...
          // Если ошибка сети при валидации - все равно переходим на ленту
          // (токены сохранены, валидацию сделаем при первом запросе к API)
          if (kDebugMode) {
            debugPrint(
              '⚠️ Ошибка валидации токена (сеть): $e - переходим на ленту',
            );
          }
        }
      }
