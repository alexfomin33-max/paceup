    ...
    if (userId != null && accessToken != null && refreshToken != null) {
      ...
      if (_hasInternet == true) {
        try {
          ...
          if (kDebugMode) {
            debugPrint('🔹 Валидация токена: ${isValid ? "валиден" : "невалиден"}');
          }

          // 🔹 Если токен невалиден и не удалось обновить - переходим на экран входа
          if (!isValid) {
            if (kDebugMode) {
              debugPrint('⚠️ Токен невалиден и не удалось обновить - переходим на экран входа');
            }
          ...
          if (kDebugMode) {
            debugPrint('⚠️ Ошибка валидации токена (сеть): $e - переходим на ленту');
          }
        }
      }
